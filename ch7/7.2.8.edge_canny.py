...
if __name__ == '__main__':
    img = cv2.imread('img/canny.jpg', cv2.IMREAD_GRAYSCALE)
    print(img.shape)
    ...
    pos_ck = np.zeros((rows, cols), np.uint8)
    canny = np.zeros((rows, cols), np.uint8)


    gaus_img = cv2.GaussianBlur(img, (3,3), 0.3)
